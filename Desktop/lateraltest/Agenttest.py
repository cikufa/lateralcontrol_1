...
class Agent:
    ...
    def choose_action(self, observation):  # obs shape (1,2)
        ...
        action = tf.tanh(action)  # action: continuous num in range(-1, 1)
    ...
    def load_models(self, ep):
        print('... loading models ...')
        self.actor.built = True
        actor = self.actor.load_weights(self.actor.checkpoint_file +f'/model{ep}.h5')
        # self.critic.load_weights(self.critic.checkpoint_file)
        # actor= tf.keras.models.load_model(self.actor.checkpoint_file)
        return actor 
